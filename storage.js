(() => {
  ...
  function importData(file, done) { const reader = new FileReader(); reader.onload = () => { try { const parsed=JSON.parse(reader.result); if (!parsed.settings || !parsed.settings.pattern) throw new Error('invalid'); const data={...defaults(),...parsed,settings:{...defaults().settings,...parsed.settings}}; save(data); done(data); } catch { alert('백업 파일을 읽지 못했어. ShiftCal 백업 JSON인지 확인해줘.'); } }; reader.readAsText(file); }
